...
export async function upsertEnvAdminUser({ email, username, passwordHash, displayName = "Administrador" }) {
  ...
  const [existingUser] = await sql`
    SELECT
      ...
      updated_at
    FROM admin_users
    WHERE (${normalizedEmail} IS NOT NULL AND LOWER(COALESCE(email, '')) = ${normalizedEmail})
      OR (${normalizedUsername} IS NOT NULL AND LOWER(COALESCE(username, '')) = ${normalizedUsername})
    LIMIT 1
  `;
